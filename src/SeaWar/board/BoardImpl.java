...
class BoardImpl implements Board, BoardViewAccess {

  /**
   * two dimensional array keeping status of each field dim 1 is column, dim 2 row
   */
  private FieldStatus[][] fieldStatus;

  BoardImpl(FieldStatus initialStatus) {
  ...
  /**
   * Should be called prior to operations on the board.Checks for wrong coordinates and throws a
   * BoardException in that case
   *
   * @param column
   * @param row
  ...
  /**
   * Shot on a field on a board: There are for cases: (1) WATER hit: no field status change, report
   * HIT WATER (2) SHIP hit: field status changes to HIT, report HIT (3) HIT hit: no field status
   * change, report WATER (a second hit is a water) (4) SHOT_ON_WATER hit: no field status change,
   * report WATER
   *
   * @param column
   * @param row
   ...
   * @return
   * @throws BoardException if following coordinate would be outside the board
   */
  static Coordinates getNextCoordinate(Coordinates currentPosition, boolean horizontal)
      throws BoardException {
    int nextColumn = currentPosition.getColumn();
