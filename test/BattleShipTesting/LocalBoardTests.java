...
public class LocalBoardTests {
  ...
  /**
   * Test if all ten ships can be set ,but not more and board notifies all ships set places.
   *
  ...
  public void testShipCoordinates() throws BoardException, GameStatusException {
    ...
    GameFactory.getGameFactory(board.getGame()).setStatus(GameStatus.PLAY_PASSIVE);
  ...
  public void shipOnTop() throws BoardException, GameStatusException {
    ...
    Ship shipLength5 = board.getUnsetShip(5);
    Ship shipLength1 = board.getUnsetShip(1);

    try {
      // coordinates 0,0 1,0 2,0 3,0 4,0
      board.putShip(shipLength5, 0, 0, true);
    } catch (BoardException e) {
      // should work ship
    }

    // on top of other - should throw BoardException
    board.putShip(shipLength1, 0, 1, true);
  }
}
